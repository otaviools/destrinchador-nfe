...
const columns: TableColumn<LinhaMatriz>[] = [
  { 
    accessorKey: 'titulo', 
    header: 'Título',
    cell: ({ row }: { row: any }) => h('span', { class: 'font-semibold text-gray-300' }, row.getValue('titulo'))
  },
  { 
    accessorKey: 'objeto', 
    header: 'Objeto (XML)',
    cell: ({ row }: { row: any }) => h('code', { class: 'px-1.5 py-0.5 rounded bg-neutral text-gray-400 font-mono text-xs border border-gray-800' }, row.getValue('objeto'))
  },
  { 
    accessorKey: 'valorObjeto', 
    header: 'Valor do Objeto (Bruto)',
    cell: ({ row }: { row: any }) => h('span', { class: 'font-mono text-gray-400 text-sm' }, row.getValue('valorObjeto'))
  },
  { 
    accessorKey: 'resultadoHumanizado', 
    header: 'Resultado Humanizado',
    cell: ({ row }: { row: any }) => h('span', { class: 'font-medium text-orange-400 text-sm' }, row.getValue('resultadoHumanizado'))
  }
]
...
const destrinchar = () => {
  ...
  dataInvertida.value = [
    { id: '1', titulo: 'Nota Fiscal',        objeto: 'nNF',    valorObjeto: numeroBruto,    resultadoHumanizado: numeroFormatado },
    { id: '2', titulo: 'Série da NF-e',       objeto: 'serie',  valorObjeto: serieBruta,     resultadoHumanizado: serieFormatada },
    { id: '3', titulo: 'Modelo da NF-e',      objeto: 'mod',    valorObjeto: modelo,         resultadoHumanizado: modelo },
    { id: '4', titulo: 'CNPJ',                objeto: 'CNPJ',   valorObjeto: cnpjBruto,      resultadoHumanizado: cnpjFormatado },
    { id: '5', titulo: 'Data Emissão',        objeto: 'AAMM',   valorObjeto: `${ano}${mes}`, resultadoHumanizado: dataEmissao },
    { id: '6', titulo: 'UF (Estado)',         objeto: 'cUF',    valorObjeto: codigoUF,       resultadoHumanizado: ufSigla },
    { id: '7', titulo: 'Tipo de Emissão',     objeto: 'tpEmis', valorObjeto: tipoEmissaoCodigo, resultadoHumanizado: tipoEmissaoTexto },
    { id: '8', titulo: 'Código Aleatório',    objeto: 'cNF',    valorObjeto: codigoAleatorio, resultadoHumanizado: codigoAleatorio },
    { id: '9', titulo: 'Dígito Verificador',  objeto: 'cDV',    valorObjeto: dv,             resultadoHumanizado: dv }
  ]
}
...
<template>
  <div class="w-full max-w-6xl px-4 py-8 mx-auto flex flex-col gap-8">
    
    <UTable
      v-if="dataInvertida.length > 0"
      :data="dataInvertida"
      :columns="columns"
      class="border border-gray-900 rounded-xl bg-neutral overflow-hidden text-white shadow-2xl"
    />
